"""
Runner script to train both WGAN-GP and Tail-GAN on S&P 500 data.

This script:
1. Downloads S&P 500 returns data
...
from wgan_gp import train_wgan_gp, Generator, Critic
from tail_gan import train_tailgan, calculate_var, calculate_es


def create_output_dir():
    """Create output directory for results."""
    output_dir = "results_sp500"
    os.makedirs(output_dir, exist_ok=True)
    return output_dir


def download_sp500(start_date='2016-01-01', end_date='2023-12-31'):
    ...
    try:
        ...
        print(f"Downloaded {len(log_returns)} daily returns")
        return log_returns
    except ImportError:
...
def create_rolling_windows(returns, window_size=50):
...
def plot_distribution_comparison(real_data, wgan_data, tailgan_data, output_dir):
    ...
    p1, p99 = np.percentile(real_flat, [1, 99])
    ...
    sorted_wgan = np.sort(wgan_flat)
    n = min(len(sorted_real), len(sorted_wgan))
    ax4.scatter(sorted_real[:n], sorted_wgan[:n], alpha=0.3, s=1)
    lims = [min(sorted_real[:n].min(), sorted_wgan[:n].min()),
            max(sorted_real[:n].max(), sorted_wgan[:n].max())]
    ...
    ax5 = axes[1, 1]
    sorted_tailgan = np.sort(tailgan_flat)
    n = min(len(sorted_real), len(sorted_tailgan))
    ax5.scatter(sorted_real[:n], sorted_tailgan[:n], alpha=0.3, s=1)
    lims = [min(sorted_real[:n].min(), sorted_tailgan[:n].min()),
            max(sorted_real[:n].max(), sorted_tailgan[:n].max())]
...
def plot_tail_comparison(real_data, wgan_data, tailgan_data, alpha, output_dir):
    ...
    tail_cutoff = np.percentile(real_flat, 15)
    bins = np.linspace(real_flat.min(), tail_cutoff, 40)

    for ax, data_flat, var, es, title in [
        (axes[0], real_flat, real_var, real_es, 'Real S&P 500'),
...
def plot_sample_paths(real_data, wgan_data, tailgan_data, output_dir, n_paths=5):
    ...
    for ax, data, title in [
        (axes[0], real_data, 'Real S&P 500 Return Paths'),
...
def main():
    print("=" * 70)
    print("IMPROVED GAN IMPLEMENTATIONS - S&P 500 DATA")
    ...
    # =========================================================================
    print("\n" + "-" * 70)
    print("Step 1: Loading S&P 500 returns data")
    print("-" * 70)

    # Download S&P 500 data
    returns = download_sp500(start_date='2016-01-01', end_date='2023-12-31')

    # Create rolling windows
    window_size = 50
    data = create_rolling_windows(returns, window_size)
    print(f"Created {data.shape[0]} rolling windows of size {window_size}")
    ...
    # Generate samples
    wgan_samples = wgan_generator.generate(data.shape[0])
    ...
    # Generate samples
    tailgan_samples = tailgan_generator.generate(data.shape[0])
    ...
    # =========================================================================
    print("\n" + "=" * 70)
    print("FINAL COMPARISON - S&P 500 TAIL RISK METRICS")
    print("=" * 70)
    ...
    # Calculate errors
    print("\n" + "-" * 56)
    print("ERRORS (relative to Real S&P 500)")
    print("-" * 56)
